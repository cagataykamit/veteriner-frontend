...
function extractFieldErrorsFromBody(body: unknown): SpeciesUpsertFieldErrors {
    const out: SpeciesUpsertFieldErrors = {};
...
function mapApiFieldKeyToFormField(rawKey: string): SpeciesUpsertFormFieldKey | null {
    const key = rawKey
        .replace(/^\$/, '')
        .replace(/\[\d+]$/, '')
        .toLowerCase()
        .replace(/[^a-z]/g, '');
    const map: Record<string, SpeciesUpsertFormFieldKey> = {
        code: 'code',
        name: 'name',
        isactive: 'isActive',
        active: 'isActive',
