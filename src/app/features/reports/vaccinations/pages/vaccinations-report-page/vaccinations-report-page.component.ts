...
import { formatDateDisplay, formatUtcIsoAsLocalDateTimeDisplay, localDateYyyyMmDd } from '@/app/shared/utils/date.utils';
        ...
        <div class="card mb-4">
            ...
            <div class="grid grid-cols-12 gap-3 items-end">
                <div class="col-span-12 md:col-span-3">
                    <label for="vacRepFrom" class="block text-xs font-medium text-muted-color mb-1">Tarih (Başlangıç)</label>
                    <input id="vacRepFrom" type="date" class="w-full p-inputtext p-component" [(ngModel)]="fromDateInput" />
                </div>
                <div class="col-span-12 md:col-span-3">
                    <label for="vacRepTo" class="block text-xs font-medium text-muted-color mb-1">Tarih (Bitiş)</label>
                    <input id="vacRepTo" type="date" class="w-full p-inputtext p-component" [(ngModel)]="toDateInput" />
                </div>
                <div class="col-span-12 md:col-span-3">
                    <label for="vacRepClinic" class="block text-xs font-medium text-muted-color mb-1">Klinik</label>
                    <p-select
                ...
                <div class="col-span-12 md:col-span-3">
                    <label for="vacRepStatus" class="block text-xs font-medium text-muted-color mb-1">{{ copy.vaccinationsReportColStatus }}</label>
                    <p-select
                ...
                <div class="col-span-12 md:col-span-6">
                    <label for="vacRepSearch" class="block text-xs font-medium text-muted-color mb-1">Arama</label>
                    <input
                        pInputText
                        id="vacRepSearch"
                        class="w-full"
                ...
                <div class="col-span-12 md:col-span-6 flex flex-wrap gap-2 justify-end">
                    ...
                    <p-button
                        [label]="copy.vaccinationsReportExportXlsx"
                    ...
                    />
                    <p-button
                        [label]="copy.vaccinationsReportExportCsv"
                        icon="pi pi-download"
                        severity="help"
                        (onClick)="exportCsv()"
            ...
            <div class="card mb-4">
                <div class="grid grid-cols-12 gap-4">
                    <div class="col-span-12 sm:col-span-6">
                        <span class="block text-muted-color text-sm mb-1">{{ copy.vaccinationsReportTotalRecords }}</span>
                        ...
                        >
                            ...
                            <ng-template #body let-row>
                                <tr>
                                    ...
                                    <td>
                                        <a [routerLink]="['/panel/vaccinations', row.id]" class="text-primary font-medium no-underline text-sm">Detay</a>
                                    </td>
                                </tr>
                            </ng-template>
                        </p-table>
                        ...
                        @for (row of displayedRows(); track row.id) {
                            <div class="rounded-border border border-surface-200 dark:border-surface-700 bg-surface-0 dark:bg-surface-900 p-4 shadow-sm">
                                ...
                                <div class="text-sm text-muted-color mb-1">
                                    <span class="font-medium">{{ copy.vaccinationsReportColAppliedAt }}: </span>{{ formatDate(row.appliedAtUtc) }}
                                </div>
                                <div class="text-sm text-muted-color mb-1">
                                    <span class="font-medium">{{ copy.vaccinationsReportColNextDue }}: </span>{{ formatDate(row.dueAtUtc) }}
                                </div>
